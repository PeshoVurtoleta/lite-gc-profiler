...
test('measureFrames: a real steady leak reads clearly above the clean floor', async () => {
    const sink = [];
    const leaky = (i) => { const o = {}; for (let k = 0; k < 30; k++) o['k' + k] = i * k; sink.push(o); };
    const r = await measureFrames(leaky, {
        frames: 300, warmup: 60, scheduler: fastSched
    });
    assert.equal(r.bytesPerFrameStable, true);
    assert.ok(r.bytesPerFrame > 800,
        'a ~1.7 KB/frame steady leak must read well above the floor; got ' + r.bytesPerFrame);
});
